...
function handleGetStudent(req, res) {
  if (req.session.user_authentication) {
    if (req.session.user_authentication.user_type === 'admin') {
      ...
      console.log("unauthorized user with invalid user type trying to get students");
      res.sendStatus(403);
    }
  }
  else {
...
function handleCreateStudent(req, res) {
  ...
  if (req.session.user_authentication) {
    if (req.session.user_authentication.user_type === 'admin') {
      ...
      console.log("unauthorized user with invalid user type trying to add student");
      res.sendStatus(403);
    }
  }
  else {
    console.log("unauthorized user with no authentication trying to add student");
    res.sendStatus(403);
  }
}
